(declaim (optimize (speed 3) (space 0) (debug 0) (safety 0))
		 (inline vec intersect sintersect ray-trace ray-sphere 
				 sray-sphere x y z))
  ...
  `(progn
	 ...
	 (defmacro ,mname ,(mapcar #'car mparams)
	   ,(loop with inner = (list name)
		   ...
		   for names = (loop repeat (the fixnum count) collect (gensym))
		   do
...
(defstruct ray
  (orig zero :type (vector double-float 3))
  (dir zero :type (vector double-float 3)))

(defstruct sphere
  (center zero :type (vector double-float 3))
          ...
          (flet ((kid (x z)
                 ...
                 (bound (c scenes)
                   (loop for s in scenes
					  maximize (+ (mmagnitude (v- c (sphere-center 
													 s)))
								  (sphere-radius s)))))
            (let ((objs (list obj
                              (kid (- rt) (- rt))
			  ...
			  (loop for dx from x below (1+ x) by (/ ss) do
				   (loop for dy from y below (1+ y) by (/ ss)
					  ...
					  (setf (values (x dir) (y dir) (z dir))
							(unitise/values dx dy (float n 
														 0d0)))
					  (incf g (ray-trace neg-light zero dir 
										 scene))))
			  (write-byte (round (* 255.0d0 (/ g (* ss ss)))) 
						  s))))))
