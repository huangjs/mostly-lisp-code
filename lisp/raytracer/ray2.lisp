(declaim (optimize (speed 3) (debug 0) (safety 0) (compilation-speed 0))
		 (inline vec x y z *v +v -v dot magnitude unitise ray-sphere intersect sray-sphere sintersect ray-trace))
...
(declaim (ftype (function (double-float vec) vec) *v)
		 (ftype (function (vec vec) vec) +v -v)
		 (ftype (function (vec vec) double-float) dot)
		 (ftype (function (vec) double-float) magnitude)
		 (ftype (function (vec) vec) unitise))
(defun *v (s a)
  (vec (* s (x a)) (* s (y a)) (* s (z a))))
(defun +v (a b)
...
(defun magnitude (a)
  (sqrt (the (double-float 0d0) (dot a a))))
(defun unitise (a)
  (*v (/ 1d0 (magnitude a)) a))
...
(declaim (ftype (function (vec vec vec double-float) double-float) ray-sphere))
...
(declaim (ftype (function (vec vec vec double-float) boolean) sray-sphere))
          ...
          (flet ((kid (x z)
                 ...
                 (bound (c scenes)
                   (loop for s in scenes
                         maximize (+ (magnitude (-v c (sphere-center 
s)))
                                     (sphere-radius s)))))
            (let ((objs (list obj
                              (kid (- rt) (- rt))
            ...
            (loop for x from -n/2 to 1-n/2
                  ...
                  (loop for dx from x below (1+ x) by (/ ss) do
                        (loop for dy from y below (1+ y) by (/ ss)
                              for dir = (unitise (vec dx dy (float n 
0d0))) do
                              (incf g (ray-trace neg-light zero dir 
scene))))
