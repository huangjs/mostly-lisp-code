...
#-sbcl (declaim (inline numeric-char=)
      ...
      (let ((other-case (gethash (first case) cases-table)))
        (if other-case
            (warn "Duplicate string-case cases: ~A -> ~A or ~A~%"
                  ...
                  (rest other-case)
                  (rest case))
            (setf (gethash (first case) cases-table) case))))
    (let ((input-var    (gensym "INPUT"))
          (default-fn   (gensym "ON-ERROR"))
          (default-body (rest (gethash t cases-table (list t default)))))
      `(let ((,input-var ,string))
         (flet ((,default-fn ()
                  ,@default-body))
           ,(emit-string-case (progn
                                (remhash t cases-table)
                                (mapcar 'rest (hash-table->list cases-table)))
                              input-var
                              `(,default-fn)))))))
