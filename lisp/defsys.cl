...
(defpackage "DEFSYS"
  (:use "COMMON-LISP")
  (:import-from "SB-PCL" "SLOTS" "NAME" "INITFUNCTION" "LOAD-DEFMETHOD")
...
(defun copy-file (from to
                  &key overwrite force remove-destination
                  &allow-other-keys
...
(defmethod compile-module ((module lisp-module) &key)
  (let* ((p (get-package module))
         (package (or (and p
                           (if (packagep p)
                               p
    ...
    (unless (or .interpreted-mode. (interpreted module))
      (progv *recurse-bind-vars* *recurse-bind-vals*
        #+sbcl (compile-file source-pathname) ; sbcl bug
        #-sbcl (compile-file source-pathname :output-file product-pathname))
...
(defun logical-merge (p &optional defaults)
  (let ((p (pathname p)))
    (when (and (sb-int:logical-pathname-p p)
...
(defmethod product-pathname ((module lisp-module))
  (logical-merge (make-pathname :type sb-int::*fasl-file-type*)
                 (source-pathname module)))
...
(defmethod shared-initialize :before ((instance defsystem-base-class)
  ...
  (let* ((class (class-of instance))
         (slotds (slot-value class 'slots)))
    (dolist (slotd slotds)
      (let ((slot-name (slot-value slotd 'name)))
        (progn
          (if (and slot-names
                   (or (eq slot-names 't)
                       (member slot-name slot-names)))
              (let ((initfunction (slot-value slotd 'initfunction)))
...
(defmethod shared-initialize :after ((new-module default-module)
  ...
  (when comp-meth-sup
    ...
    (load-defmethod 'compile-module :qualifiers 'nil
         :specializers `(,(mop:intern-eql-specializer new-module))      ;bug2989
