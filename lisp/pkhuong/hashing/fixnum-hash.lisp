...
(defstruct (hash
             (:constructor %make-hash (size data
                                            &key (count 0) (cleared 0) (load .5d0)
...
(defun magic-values (size &key mix data)
  ...
  (let* ((mix   (or mix (make-mix-value)))
         ...
         (len    (* 3 (+ size 5)))
         (data (if (and data
                        (typep data '(simple-array fixnum 1))
                        (>= (length data) len))
                   (fill data 0)
...
(defconstant +deleted-key+ 1)

(declaim (inline hash-info)
         (ftype (function (hash-vector fixnum &optional (or null positive-fixnum))
                          (values fixnum index &optional))
                hash-info))
(defun hash-info (data key &optional hash)
  (declare (type hash-vector data)
           (optimize speed))
  (let* ((mix   (aref data 0))
         (scale (aref data 1))
         (hash  (or hash (logand (mix key mix)
                                 most-positive-fixnum)))
         (loc   (* 3 (truly-the positive-fixnum (1+ (scale-by hash scale))))))
    (declare (type positive-fixnum loc))
    (values hash loc)))
...
(defun %hash-get (table key)
  ...
  (let* ((data  (hash-data   table))
         (len   (aref data 2)))
    (multiple-value-bind (hash loc) (hash-info data key)
      ...
      (loop for i from loc below len by 3
         ...
         (let ((h2 (aref data i))
           ...
           (cond ((zerop k2)
                 ...
                 ((eql key k2)
                  (let ((value (aref data (+ i 2))))
                    (setf (aref data (+ i 1)) +deleted-key+)
                    (values value t)))
                 (t (return (values nil nil)))))))))
