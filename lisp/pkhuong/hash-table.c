...
static void
resize_bucket (bucket_t * bucket, size_t new_max)
{
...
hash_table_set (table_t * table, size_t addr, table_value_t value)
{
        ...
        for (size_t i = 0; i < ndescriptors; i++) {
                if (top == bucket->tops[i])
                        return descriptor_add(&bucket->descriptors[i],
                                              middle, value);
        }

        if (ndescriptors >= bucket->max_descriptors) {
                size_t new_max = ndescriptors*2;
                resize_bucket(bucket, (new_max < 2) ? 2 : new_max);
        }

        bucket->tops[ndescriptors] = top;
        init_descriptor(&bucket->descriptors[ndescriptors], middle, value);

        bucket->ndescriptors = ndescriptors+1;

        return 0;
}

static inline void
rotate_descriptor (size_t * top, page_descriptor_t * descriptor,
                   bucket_t * bucket)
{
        size_t ndescriptors = bucket->ndescriptors--;
        *top        = bucket->tops[ndescriptors];
        *descriptor = bucket->descriptors[ndescriptors];
        
        if (ndescriptors*2 < bucket->max_descriptors)
                resize_bucket(bucket, ndescriptors);
}
...
hash_table_del (table_t * table, size_t addr)
{
        ...
        if (top == bucket->descriptor_top) {
                ...
                if (old && !bucket->descriptor.values) {
                        ...
                        if (0 == ndescriptors)
                                ...
                                rotate_descriptor(&bucket->descriptor_top,
                                                  &bucket->descriptor,
                                                  bucket);
                }
        ...
        for (size_t i = 0; i < ndescriptors; i++) {
                if (top != bucket->tops[i]) continue;

                table_value_t old
                        = descriptor_del(&bucket->descriptors[i],
                                         middle);
                if (old && !bucket->descriptors[i].values)
                        rotate_descriptor(bucket->tops+i,
                                          bucket->descriptors+i,
                                          bucket);
