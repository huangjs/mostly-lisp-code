...
(DEFMETHOD GET-KEYS              ((SELF DICTIONARY))
  (MAPCAR (FUNCTION CAR) (ASSOCIATIONS SELF))) ;;GET-KEYS

(DEFMETHOD GET-VALUES            ((SELF DICTIONARY))
  (MAPCAR (FUNCTION CDR) (ASSOCIATIONS SELF))) ;;GET-VALUES
  ...
  (COND
    ...
    (T	
     (DICTIONARY-PRIVATE-ASSOCIATION-AT-KEY 
      (CDR ASSOCIATIONS) KEY)))) ;;DICTIONARY-PRIVATE-ASSOCIATION-AT-KEY
...
(DEFMETHOD VALUE-AT-KEY           ((SELF DICTIONARY) KEY)
  (LET ((ASSOC 
    ...
    (IF (NULL ASSOC)
        ASSOC
        (CDR ASSOC)))) ;;VALUE-AT-KEY

(DEFMETHOD ASSOCIATION-AT-KEY     ((SELF DICTIONARY) KEY)
  (DICTIONARY-PRIVATE-ASSOCIATION-AT-KEY (ASSOCIATIONS SELF) KEY)) ;;ASSOCIATION-AT-KEY
...
(DEFMETHOD PUT-AT-KEY             ((SELF DICTIONARY) KEY VALUE)
  (LET ((ASSOC 
    ...
    (COND
      ...
      (T
       (RPLACD ASSOC VALUE))))) ;;PUT-AT-KEY
	
(DEFMETHOD CONTAINS-ASSOCIATION? ((SELF DICTIONARY) ASSOCIATION)
  (MEMBER ASSOCIATION (ASSOCIATIONS SELF))) ;;CONTAINS-ASSOCIATION?
	
(DEFMETHOD CONTAINS-KEY?         ((SELF DICTIONARY) KEY)
  (MEMBER KEY (MAPCAR 'CAR (ASSOCIATIONS SELF)))) ;;CONTAINS-KEY?

(DEFMETHOD APPLY-TO-ASSOCIATIONS  ((SELF DICTIONARY) FUNC)
  (DECLARE (TYPE (FUNCTION (CONS) T) FUNC))
  (MAPCAR FUNC (ASSOCIATIONS SELF))) ;;APPLY-TO-ASSOCIATIONS
