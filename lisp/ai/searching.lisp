...
;;; typical strategy: depth first search
;;; we generate the successors of a state, and then work on the first successor first.
;;; the combiner is simple: append
(defun depth-first-search (start goal? successors)
  "Search new states first until goal is reached."
  (tree-search (list start) goal? successors #'append))

;;; examples
(defun number-binary-tree (x) (list (* 2 x) (+ 1 (* 2 x))))
...
but never consider more than beam-width states at a time."
  (tree-search (list start) goal? successors
               #'(lambda (old new)
                   (let ((sorted (funcall (cost-sort cost-fn) old new)))
...
;;; !! FIXME
(defun adaptive-beam-search (start goal? successors cost-fn beam-width-diff-fn)
  "Beam search with adaptive beam width."
  (tree-search (list start) goal? successors
               #'(lambda (old new)
                   (let* ((sorted (funcall (cost-sort cost-fn) old new))
                          (old-width (length old))
                          (new-width (length new))
                          (beam-width-diff (funcall beam-width-diff-fn old-width new-width))
                          (beam-width (+ old-width beam-width-diff)))
                     (if (> beam-width (length sorted))
                         sorted
                         (subseq sorted 0 beam-width))))))
...
;;; iterative widening beam search
...
;;; search all the solutions based on beam-search
;;; if you really want all the solutions, provide beam-width with most-positive-long-float
;;; but be careful that it may not terminate.
;;; most-positive-fixnum is a good choice.
...
Don't try the same state twice."
  ...
  (cond ((null states) fail)
        ((funcall goal? (first states)) (first states))
        (t (graph-search
            (funcall combiner (new-states states successors state= old-states)
                     (rest states))
